    ...
    <x-card>
        ...
        <div>
            <form enctype="multipart/form-data"
                action="{{route(config('theme.rprefix') . '.assign.student.store', $classes->id) }}"
                ...
                <div class=" row">
                    <div class="col-md-12">
                        ...
                        <hr>
                        <div class="form-group pt-1 pb-1">
                            <h5 for="name" class="font-black">{{ __('Student list') }}</h5>
                            
                            @foreach ($students as $student)
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="student_id" name="student_id[]" value="{{ $student->id }}">
                                <label class="custom-control-label" for="student_id">{{ $student->name }}</label>
                            </div>
                            @endforeach
                            @error('name')
